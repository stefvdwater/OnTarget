...
### Print-overzicht: filter-labels, page-breaks en open intervallen (PR-G uit openstaande werken)

Drie samenhangende correcties aan de Afdrukken-tab en het print-document, zonder wijziging aan de indeling of het databaseschema. Bestaande afdruk-instellingen blijven werken.

- **Doel-labels volgen werkelijke positie** (P6-1): de print-doel-tabel labelde voorheen elke rij met `idx + 1` van een gesorteerde lijst, niet met de werkelijke `slot.positie` van de schutter. Bij een onvolledig doel of bij een actief filter belandde een schutter zo onder een fout label (bv. werkelijke positie 3 verscheen als "1A"). [`DoelTabel`](../src/renderer/src/components/PrintDocument.tsx) gebruikt nu twee modi: bij gilde-filter (`filters.gildes !== 'alle'`) compact – alleen de passerende schutters worden getoond, met hun werkelijke positie in het label; in alle andere gevallen (geen filter, doel-filter of afstand-filter) blijven 6 vaste rijen per doel, waarbij elke schutter op de rij van zijn `slot.positie` komt en gaten leeg blijven. De gilde-tabel kreeg dezelfde correctie: `positie1` is nu altijd `slot.positie` in plaats van een index over een gefilterde lijst.
- **Doel-blokken splitsen niet meer over pagina's** (P6-2): de print-tabel had één enkele `<tbody>` met een algemene `tr { page-break-inside: avoid }`, wat een doel-blok midden zijn A..F-rijen kon laten breken. Elk doel zit nu in een eigen `<tbody class="print-doel-groep">`. In [`index.css`](../src/renderer/src/index.css) krijgt die groep `page-break-inside: avoid` binnen `@media print`, zodat een doel altijd als één blok op één pagina afgedrukt wordt. De gilde-tabel raakt deze wijziging niet.
- **Open intervallen in de doel-selectie** (uit P6-3-discussie): [`parseDoelInterval`](../src/renderer/src/components/afdruk-helpers.ts) accepteert nu open uiteinden in een interval-uitdrukking. `2-` betekent vanaf doel 2 tot en met het laatste doel; `-3` betekent doel 1 tot en met 3; `-` op zichzelf is gelijk aan alle doelen. Bestaande syntax (`1-10, 15`) blijft ongewijzigd. Foutpaden (omgekeerd interval, buiten bereik, niet-numeriek) gedragen zich identiek.
