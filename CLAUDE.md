...
- **Changelog-template meteen na de release.** De release zelf zet ook al een lege `internal-docs/RELEASE_UNRELEASED.md` klaar op `main`, zodat de eerste feature-PR van de nieuwe cyclus die template aanvult in plaats van er zelf een op te zetten. Geen versienummer gokken: dat bestand heet pas `RELEASE_<versie>.md` bij de eerstvolgende release zelf (stap 4 in de skill). Zie stap 9 in de [`/release`-skill](.claude/skills/release/SKILL.md).
